...
void drawFrame(EngineData* data, const std::vector<Model>& models) {
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || data->windowData.framebufferResized) {
        ...
        createSyncObjects(data->vulkanData.device, &data->renderData.syncResources, data->MAX_FRAMES_IN_FLIGHT);
        return;
    }
    ...
    for (uint32_t i = 0; i < models.size(); i++) {
        ...
        vkCmdBindVertexBuffers(
            data->renderData.commandResources.buffers[data->currentFrame], 0, 1, &models[i].vertexBuffer.buffer, offsets);
